...
int main (int argc, char *argv[])
{
	...
	try
	{
		...
		if (!c.is_valid())
			throw string("Commandline could not be read, Usage : gencarp instance -type problemType [-t cpu-time] [-sol solutionPath]  [-s seed] [-veh nbVehicles] [-dep nbDepots] [-tcap 0|1] [-soft 0|1] [-dhead 0|1]");
		...
		if (!minFleetSize && !minMaxTour)
		{
			...
			mesParametres = new Params(c.get_path_to_instance(),c.get_path_to_solution(),c.get_path_to_BKS(),c.get_seed(),c.get_type(),c.get_timeCapacitated(),c.get_softConstraints(),c.get_nbVeh(),c.get_nbDep(),false,c.get_deadheadingArcs()) ;
		...
		else
		{
			...
			{	
				...
				mesParametresTab.push_back(new Params(c.get_path_to_instance(),c.get_path_to_solution(),c.get_path_to_BKS(),c.get_seed(),c.get_type(),c.get_timeCapacitated(),c.get_softConstraints(),veh,c.get_nbDep(),true,c.get_deadheadingArcs())) ;
			...
			// Thus, a last optimization run is done
			{
				...
				mesParametres2 = new Params(c.get_path_to_instance(),c.get_path_to_solution(),c.get_path_to_BKS(),c.get_seed(),c.get_type(),c.get_timeCapacitated(),c.get_softConstraints(),veh,c.get_nbDep(),false,c.get_deadheadingArcs()) ;
